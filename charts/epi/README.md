...
A Persistent Volume is mounted to the Builder and Runner Pod. Therefore a PersistentVolumeClaim (PVC) is being deployed by the helm chart at hook `pre-install` with various configuration options (see [values.yaml](values.yaml) at section `persistence`). The PVC is being deleted by *Cleanup Job* on deletion of the helm chart if `persistence.deletePvcOnUninstall: true` (default).
...
| builder.image.sha | string | `"7756930eecac2364ba66d052839bab512dbc5f423671c13aa5e368b313f61f60"` | sha256 digest of the image for the builder. Do not add the prefix "@sha256:" Default to v1.3.1 |
| builder.image.tag | string | `"1.3.1-rc"` | Image tag for the builder. Default to v1.3.1 |
...
| runner.image.sha | string | `"6a0dbf0a2d54490235b303f97cbe5d833e6f1a69ac306e362a6ba72ef9c9c38a"` | sha256 digest of the image. Do not add the prefix "@sha256:" Default to v1.3.1 |
